...
router.put('/output/:id', webAuth, async (req, res) => {
    try {
        ...
        const [existing] = await pool.execute('SELECT id FROM output_mc WHERE id = ?', [id]);
        if (existing.length === 0) {
            return res.status(404).json({
                success: false,
...
router.delete('/output/:id', webAuth, async (req, res) => {
    try {
        ...
        await pool.execute('DELETE FROM output_mc WHERE id = ?', [id]);
...
router.get('/dashboard/stats', webAuth, async (req, res) => {
    try {
        ...
        // Quality statistics
        const [qualityStats] = await pool.execute(`
            SELECT 
...
router.get('/wip/summary', webAuth, async (req, res) => {
    try {
        ...
        const [outputTotals] = await pool.execute(`
            SELECT 
                customer,
                partnumber, 
                model,
...
router.delete('/machine-status/:id', webAuth, async (req, res) => {
    try {
        ...
        const [result] = await pool.execute('DELETE FROM machine_status WHERE id = ?', [id]);
...
router.delete('/outputmc/:id', webAuth, async (req, res) => {
    try {
        ...
        const [result] = await pool.execute('DELETE FROM outputmc WHERE id = ?', [id]);
...
router.delete('/transferqc/:id', webAuth, async (req, res) => {
    try {
        ...
        const [result] = await pool.execute('DELETE FROM transferqc WHERE id = ?', [id]);
...
router.delete('/wipsecond/:id', webAuth, async (req, res) => {
    try {
        ...
        const [result] = await pool.execute('DELETE FROM wipsecond WHERE id = ?', [id]);
