...
class DashboardService {
    ...
    async getProductionDashboard() {
        try {
            ...
            const [machineEfficiency] = await pool.execute(`
                SELECT 
                    o.machine,
                    COUNT(*) as total_runs,
                    SUM(o.quantity) as total_good,
            ...
            const [operatorPerformance] = await pool.execute(`
                SELECT 
                    o.operator,
                    COUNT(*) as total_runs,
                    SUM(o.quantity) as total_good,
    ...
    async getWarehouseDashboard() {
        try {
            ...
            const [stockMovement] = await pool.execute(`
                SELECT 
                    'WIP' as stock_type,
    ...
    async getQualityDashboard() {
        try {
            ...
            const [qualityMetrics] = await pool.execute(`
                SELECT 
                    o.partnumber,
                    o.customer,
                    o.model,
            ...
            const [ngByMachine] = await pool.execute(`
                SELECT 
                    o.machine,
                    SUM(o.quantity) as good_quantity,
                    SUM(o.quantity_ng) as ng_quantity,
    ...
    async getPlanningDashboard() {
        try {
            ...
            const [scheduleVsActual] = await pool.execute(`
                SELECT 
                    s.partnumber,
                    s.description,
                    s.daily_schedule,
